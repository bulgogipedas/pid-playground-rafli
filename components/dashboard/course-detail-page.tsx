...
export function CourseDetailPage({ course }: { course: Course }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-[28px] border border-border bg-card shadow-[0_28px_80px_rgba(20,24,33,0.14)] dark:shadow-[0_28px_80px_rgba(0,0,0,0.28)]">
        <div className="grid lg:grid-cols-[1.08fr_0.92fr]">
          ...
          <div className="flex flex-col justify-center p-5 sm:p-8 lg:p-10">
            ...
            <h1 className="text-balance font-serif text-3xl font-medium leading-[1.02] tracking-[-0.045em] text-foreground sm:text-4xl lg:text-[44px]">{course.title}</h1>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2">
              <div className="rounded-[14px] border border-border bg-muted/60 p-3"><Clock className="mb-2 h-4 w-4 text-blue-600" /><p className="text-sm font-medium text-foreground">{formatDuration(course.duration)}</p><p className="mt-0.5 text-[11px] text-muted-foreground">Durasi</p></div>
              <div className="rounded-[14px] border border-border bg-muted/60 p-3"><Users className="mb-2 h-4 w-4 text-success" /><p className="text-sm font-medium text-foreground">{course.enrollmentCount}</p><p className="mt-0.5 text-[11px] text-muted-foreground">Peserta</p></div>
              <div className="rounded-[14px] border border-border bg-muted/60 p-3"><Star className="mb-2 h-4 w-4 fill-warning text-warning" /><p className="text-sm font-medium text-foreground">{course.rating}</p><p className="mt-0.5 text-[11px] text-muted-foreground">Rating</p></div>
            </div>

            {enrollment && (
              <div className="mt-5 rounded-[16px] border border-emerald-400/15 bg-emerald-400/[0.07] p-4">
                <div className="mb-2 flex items-center justify-between text-sm"><span className="text-success">Progress Anda</span><span className="font-medium text-foreground">{enrollment.progress}%</span></div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-6">
          <Card className="border-border bg-card py-0"><CardContent className="p-5 sm:p-7">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-[16px] border border-border bg-muted/50 p-4"><Layers3 className="mb-3 h-5 w-5 text-violet-500" /><p className="font-medium">{course.sections.length} bagian</p><p className="mt-1 text-xs text-muted-foreground">Urutan belajar terstruktur</p></div>
              <div className="rounded-[16px] border border-border bg-muted/50 p-4"><BookOpen className="mb-3 h-5 w-5 text-blue-500" /><p className="font-medium">{totalMaterials} materi</p><p className="mt-1 text-xs text-muted-foreground">Video, bacaan, dan aktivitas</p></div>
              <div className="rounded-[16px] border border-border bg-muted/50 p-4"><CheckCircle2 className="mb-3 h-5 w-5 text-success" /><p className="font-medium">{course.completionRate}%</p><p className="mt-1 text-xs text-muted-foreground">Tingkat penyelesaian</p></div>
            </div>
          </CardContent></Card>

          <Card id="kurikulum" className="scroll-mt-24 border-border bg-card py-0"><CardContent className="p-5 sm:p-7">
                ...
                return (
                  <section key={section.id} className="py-6 first:pt-5 last:pb-0">
                    <div className="flex items-start gap-3">
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-semibold text-primary-foreground">{String(sectionIndex + 1).padStart(2, "0")}</span>
                      <div className="min-w-0 flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><h3 className="font-medium text-foreground">{section.title}</h3><span className="shrink-0 text-xs text-muted-foreground">{formatDuration(sectionDuration)}</span></div>
                        <div className="mt-4 grid gap-2">
                          {section.items.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-[12px] bg-muted/60 px-3 py-2.5 text-sm"><span className="text-muted-foreground"><MaterialIcon type={item.type} /></span><span className="min-w-0 flex-1 text-foreground">{item.title}</span><span className="shrink-0 text-xs text-muted-foreground">{item.duration} mnt</span></div>)}
                        </div>
                      </div>
                    </div>
                  </section>
        ...
        <aside className="space-y-4 lg:sticky lg:top-24">
          <Card className="border-border bg-card py-0"><CardContent className="p-5">
