...
function FeaturedLearningPath() {
  ...
  return (
    <Card className="featured-course-shell overflow-hidden rounded-[26px] border-border bg-card py-0 shadow-[0_24px_80px_rgba(0,0,0,0.16)] dark:shadow-[0_24px_80px_rgba(0,0,0,0.22)]">
      <div className="featured-course-grid grid gap-0">
        ...
        <div className="flex min-w-0 flex-col justify-center gap-4 bg-[#111] p-4 text-white sm:p-5 lg:p-6">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="rounded-[14px] border border-white/[0.06] bg-white/[0.05] p-3"><p className="text-lg font-medium">{pythonCourse.modules.length}</p><p className="text-[11px] text-white/55">Tahap</p></div>
            <div className="rounded-[14px] border border-white/[0.06] bg-white/[0.05] p-3"><p className="text-lg font-medium">{videoCount}</p><p className="text-[11px] text-white/55">Video</p></div>
            <div className="rounded-[14px] border border-white/[0.06] bg-white/[0.05] p-3"><p className="text-lg font-medium">{activityCount}</p><p className="text-[11px] text-white/55">Aktivitas</p></div>
          </div>
...
function CourseCard({ course, isEnrolled, enrollment }: {
  ...
  return (
    <Card className="group flex flex-col overflow-hidden border-border bg-card transition duration-300 hover:-translate-y-1 hover:border-ring/40 hover:shadow-[0_22px_55px_rgba(20,24,33,0.14)] dark:hover:shadow-[0_22px_55px_rgba(0,0,0,0.28)]">
      ...
      <CardContent className="flex flex-1 flex-col p-4">
        ...
        <Link href={detailHref} className="rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><h3 className="line-clamp-2 min-h-12 text-base font-medium leading-6 tracking-[-0.02em] transition-colors group-hover:text-primary">{course.title}</h3></Link>
        ...
        {isEnrolled && enrollment && <div className="mb-3 space-y-2 rounded-[14px] bg-muted/60 p-3"><div className="flex justify-between text-xs"><span className="text-muted-foreground">Progress Anda</span><span className="font-medium">{enrollment.progress}%</span></div><Progress value={enrollment.progress} className="h-1.5" /></div>}
...
function CourseListItem({ course, isEnrolled, progress }: { course: Course; isEnrolled: boolean; progress?: number }) {
  const detailHref = `/dashboard/katalog/${course.id}`
  return (
    <Card className="border-border bg-card transition duration-300 hover:border-ring/40 hover:shadow-[0_18px_45px_rgba(20,24,33,0.12)] dark:hover:shadow-[0_18px_45px_rgba(0,0,0,0.24)]">
      <CardContent className="p-4"><div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1">
          ...
          <Link href={detailHref} className="rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><h3 className="line-clamp-1 font-medium tracking-[-0.02em] hover:text-primary">{course.title}</h3></Link>
...
export default function KatalogPage() {
  ...
  const statItems = [
    { label: "Total Pelatihan", value: stats.total, icon: BookOpen, className: "bg-primary/10 text-primary" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{statItems.map((item) => { const Icon = item.icon; return <Card key={item.label} className="border-border bg-card"><CardContent className="p-4"><div className="flex items-center gap-3"><div className={cn("flex h-10 w-10 items-center justify-center rounded-[12px]", item.className)}><Icon className="h-5 w-5" /></div><div><p className="text-2xl font-semibold">{item.value}</p><p className="text-xs text-muted-foreground">{item.label}</p></div></div></CardContent></Card> })}</div>

      <Card className="border-border bg-card"><CardContent className="p-4"><div className="flex flex-col gap-4 lg:flex-row">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          ...
          <div className="flex justify-self-start rounded-full border border-border p-0.5"><Button variant={viewMode === "grid" ? "secondary" : "ghost"} size="icon" onClick={() => setViewMode("grid")} aria-label="Tampilan grid" aria-pressed={viewMode === "grid"}><Grid3X3 /></Button><Button variant={viewMode === "list" ? "secondary" : "ghost"} size="icon" onClick={() => setViewMode("list")} aria-label="Tampilan daftar" aria-pressed={viewMode === "list"}><List /></Button></div>
        </div>
